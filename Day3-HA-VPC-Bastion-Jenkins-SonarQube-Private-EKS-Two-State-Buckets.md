...
```mermaid
flowchart TB
    USER["Administrator Laptop<br/>SSH 22 from approved /32<br/>Jenkins 8080 and SonarQube 9000 from approved /32"]
    INTERNET(("Internet"))
    IGW["Internet Gateway<br/>openhelp-prod-igw"]

    subgraph AWS["AWS Region: us-east-1"]
        subgraph STATE["Terraform State Layer"]
            KMS1["KMS Key<br/>EC2 state encryption"]
            S3A[("S3 Bucket 1<br/>EC2/VPC state<br/>ec2/terraform.tfstate<br/>Versioning + KMS + Lockfile")]
            KMS2["KMS Key<br/>EKS state encryption"]
            S3B[("S3 Bucket 2<br/>EKS state<br/>eks/terraform.tfstate<br/>Versioning + KMS + Lockfile")]
            KMS1 --> S3A
            KMS2 --> S3B
        end
        ...
        subgraph VPC["VPC 10.0.0.0/16"]
            ...
            subgraph AZ1["Availability Zone us-east-1a"]
                subgraph PUB1["Public Subnet 1<br/>10.0.1.0/24"]
                    BASTION["EC2: Bastion/Kubectl<br/>t3.small<br/>Public + Private IP<br/>AWS CLI, kubectl, Helm, Terraform"]
                    NAT1["NAT Gateway 1<br/>Elastic IP 1"]
                end

                subgraph PRIV1["Private Subnet 1<br/>10.0.3.0/24"]
                    NODE1["EKS Worker 1"]
                    NODE2["EKS Worker 2"]
                    PRT1["Private RT 1<br/>0.0.0.0/0 → NAT 1"]
                end
            end

            subgraph AZ2["Availability Zone us-east-1b"]
                subgraph PUB2["Public Subnet 2<br/>10.0.2.0/24"]
                    TOOLS["EC2: Jenkins + SonarQube<br/>t3.large recommended<br/>Public + Private IP<br/>Ports 22, 8080, 9000 restricted"]
                    NAT2["NAT Gateway 2<br/>Elastic IP 2"]
                end

                subgraph PRIV2["Private Subnet 2<br/>10.0.4.0/24"]
                    NODE3["EKS Worker 3"]
                    NODE4["EKS Worker 4"]
                    PRT2["Private RT 2<br/>0.0.0.0/0 → NAT 2"]
                end
            end

            EKSCP["Amazon EKS Control Plane<br/>AWS managed<br/>Public endpoint restricted to admin CIDRs<br/>Private endpoint enabled"]
            ALB["Future Internet-facing ALB<br/>Public subnets"]
        end
    end

    USER -->|SSH| BASTION
    USER -->|HTTPS/HTTP lab access| TOOLS
    USER --> INTERNET
    INTERNET --> IGW
    IGW --> PUBRT
    PUBRT --> PUB1
    ...
    PRIV1 --> PRT1 --> NAT1 --> PUBRT
    PRIV2 --> PRT2 --> NAT2 --> PUBRT

    BASTION -->|kubectl / AWS API| EKSCP
    TOOLS -->|CI/CD deploy| EKSCP
    EKSCP --> NODE1
    EKSCP --> NODE2
    EKSCP --> NODE3
    EKSCP --> NODE4
    ...
    classDef network fill:#dbeafe,stroke:#2563eb,color:#111827,stroke-width:2px;
    classDef public fill:#dcfce7,stroke:#16a34a,color:#111827,stroke-width:2px;
    classDef private fill:#fef3c7,stroke:#d97706,color:#111827,stroke-width:2px;
    classDef compute fill:#ede9fe,stroke:#7c3aed,color:#111827,stroke-width:2px;
    classDef state fill:#fee2e2,stroke:#dc2626,color:#111827,stroke-width:2px;
    classDef managed fill:#cffafe,stroke:#0891b2,color:#111827,stroke-width:2px;

    class IGW,PUBRT,PRT1,PRT2,NAT1,NAT2 network;
    class PUB1,PUB2,BASTION,TOOLS,ALB public;
    class PRIV1,PRIV2,NODE1,NODE2,NODE3,NODE4 private;
    class BASTION,TOOLS,NODE1,NODE2,NODE3,NODE4 compute;
    class S3A,S3B,KMS1,KMS2 state;
    class EKSCP managed;
```
