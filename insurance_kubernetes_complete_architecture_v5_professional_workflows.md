...
Reference design based on the uploaded architecture diagram. This document explains the production Kubernetes architecture for an insurance company serving about **1 million customers**, including microservices, Kubernetes object types, pod sizing, hardware, CI/CD, monitoring, logging, database connectivity, certificate automation, authentication, and end-to-end traffic flows.

> Assumption: Oracle Database, Bitbucket/GitLab, Jenkins, SonarQube, Nexus, **Venafi Trust Protection Platform / Venafi Control Plane connector VM**, and **Active Directory Domain Controllers** run on VMware VMs. Rancher, Argo CD, monitoring, logging, ingress, **Jetstack cert-manager**, and insurance microservices run inside Kubernetes. This version uses **NGINX Ingress Controller as the main routing gateway** and removes the separate API Gateway layer from the first architecture view.

---

## 1. High-Level Architecture

This architecture uses a simple and production-friendly Kubernetes traffic model:
...
Public DNS
        ↓
F5 HA Pair / WAF / External Load Balancer
        ↓
...
ClusterIP Services
        ↓
Microservice Deployments and Pods
        ↓
Oracle RAC / External APIs / AD / Payment Gateway
```

NGINX Ingress Controller is used as the main entry and routing layer. It terminates or passes TLS based on policy, applies host/path routing, and forwards traffic to Kubernetes ClusterIP Services. The Kubernetes Service then load-balances to the correct pods such as `customer-service`, `policy-service`, `claims-service`, and `billing-service`.
...
flowchart LR
    ...
    Users[Customers / Agents / Brokers]:::user --> DNS[Public DNS<br/>insurance.company.com]:::edge
    Devs[Developers Machines]:::user --> Git[Bitbucket / GitLab VM<br/>Source Code Repository]:::cicd
    DevOps[DevOps Engineer]:::user --> Jump[Jump Server / Bastion VM<br/>kubectl / helm / ssh]:::sec

    DNS --> F5[F5 HA Pair / WAF<br/>External Load Balancer]:::edge
    F5 --> LBSVC[NGINX Ingress Controller<br/>Service type: LoadBalancer<br/>VIP / LB IP]:::edge
    LBSVC --> NGINX[NGINX Ingress Controller Pods x3<br/>TLS + path-based routing]:::kube

    subgraph K8S[Kubernetes Production Cluster - 3 Control Plane + 10 Worker Nodes]
      CP[Control Plane x3<br/>API Server / Scheduler / Controller / etcd]:::kube
      NGINX --> Customer[customer-service<br/>Deployment x4<br/>ClusterIP Service]:::app
      NGINX --> Policy[policy-service<br/>Deployment x4<br/>ClusterIP Service]:::app
      NGINX --> Claim[claims-service<br/>Deployment x6<br/>ClusterIP Service]:::app
      NGINX --> Billing[billing-service<br/>Deployment x3<br/>ClusterIP Service]:::app
      NGINX --> Quote[quote-service<br/>Deployment x3<br/>ClusterIP Service]:::app
      NGINX --> Fraud[fraud-detection-service<br/>Deployment x4<br/>ClusterIP Service]:::app
      NGINX --> Docs[document-service<br/>Deployment x3<br/>ClusterIP Service]:::app
      NGINX --> Notify[notification-service<br/>Deployment x3<br/>ClusterIP Service]:::app
      NGINX --> Auth[auth-service<br/>Deployment x3<br/>ClusterIP Service]:::app
      ...
      CertMgr[Jetstack cert-manager x3]:::sec
      Issuer[Venafi ClusterIssuer]:::sec
      TLSSecret[Kubernetes TLS Secrets]:::sec
      Rancher[Rancher x3]:::kube
      Argo[Argo CD<br/>GitOps Deployment]:::kube
      Prom[Prometheus StatefulSet x2]:::obs
      Graf[Grafana Deployment x2]:::obs
      Alert[Alertmanager StatefulSet x3]:::obs
      Fluent[Fluentd DaemonSet<br/>1 pod per worker node]:::obs
      ES[Elasticsearch StatefulSet x6]:::obs
      Kib[Kibana Deployment x2]:::obs
    end

    Auth --> AD[Active Directory DC VM x2<br/>LDAPS 636 / Groups]:::sec
    Rancher --> AD

    Git --> Jenkins[Jenkins CI VM<br/>Build / Test / Docker Image]:::cicd
    Jenkins --> Sonar[SonarQube VM<br/>Code Quality Gate]:::cicd
    Jenkins --> Nexus[Nexus Repository VM<br/>Docker Registry / Maven / Helm Artifacts]:::cicd
    Nexus --> Argo
    Argo --> K8S
    Jump --> CP
    Jump --> Rancher

    CertMgr --> Issuer --> Venafi[Venafi VM<br/>Certificate Policy / Approval / Inventory]:::sec
    Venafi --> CA[Enterprise CA / Microsoft CA]:::sec
    CertMgr --> TLSSecret
    TLSSecret --> NGINX
    ...
    Claim --> DBLB
    Billing --> DBLB
    Payment --> Bank[Bank / Payment Gateway API]:::ext
...
This design is used because it separates enterprise edge security, Kubernetes routing, application processing, platform management, and external integrations. F5/WAF protects and load-balances the external entry point. NGINX Ingress provides Kubernetes-native HTTP routing. Each microservice runs as a Deployment behind a stable ClusterIP Service. Oracle remains outside Kubernetes for enterprise database stability. AD provides centralized identity. Venafi and cert-manager automate certificate lifecycle. Jenkins, SonarQube, Nexus, and Argo CD provide controlled CI/CD and GitOps deployment.

---

## 1.1 Step-by-Step Workflow 1 - User Traffic Routing
...
flowchart LR
    ...
    User[Customer / Agent]:::user --> DNS[DNS<br/>insurance.company.com]:::edge
    DNS --> F5[F5 HA Pair / WAF]:::edge
    F5 --> LBIP[NGINX LoadBalancer IP<br/>192.168.x.x or VIP]:::edge
    LBIP --> Nginx[NGINX Ingress Pods x3]:::kube
    Nginx --> Rule[Ingress Rule<br/>/api/customers or /api/policies]:::kube
    Rule --> Svc[ClusterIP Service]:::kube
    Svc --> Pod[Microservice Pod]:::app
    Pod --> DB[Oracle RAC / External API]:::db
```

**How routing works:**

1. User opens `https://insurance.company.com/api/customers/profile`.
2. DNS points the public name to the F5/WAF VIP.
3. F5 forwards traffic to the Kubernetes NGINX Ingress LoadBalancer IP.
4. NGINX checks the Ingress path rule.
5. `/api/customers` routes to `customer-service`.
6. `customer-service` ClusterIP load-balances to one of the 4 customer-service pods.
7. The pod connects to Oracle RAC or other internal systems as required.

---

## 1.2 Step-by-Step Workflow 2 - CI/CD Pipeline
...
flowchart LR
    ...
    Dev[Developer Machine]:::user --> Git[Bitbucket / GitLab VM<br/>Code Commit]:::cicd
    Git --> Jenkins[Jenkins CI VM<br/>Build + Unit Test]:::cicd
    Jenkins --> Sonar[SonarQube VM<br/>Quality Gate]:::cicd
    Sonar --> Image[Docker Image Build]:::cicd
    Image --> Nexus[Nexus Repository VM<br/>Docker + Maven + Helm]:::cicd
    Nexus --> GitOps[GitOps Repo<br/>Helm values image tag]:::cicd
    GitOps --> Argo[Argo CD]:::kube
    Argo --> Deploy[Kubernetes Deployment<br/>Rolling Update]:::app
```

**Why these tools are used:**

| Tool | Purpose in architecture |
|---|---|
| Bitbucket / GitLab | Stores source code, Helm charts, Kubernetes manifests, and GitOps configuration |
| Jenkins | Builds application, runs tests, creates Docker image, and updates deployment metadata |
| SonarQube | Enforces code quality, bug detection, security hotspot review, and quality gate |
| Nexus | Stores Docker images, Maven artifacts, Helm charts, release packages, and raw scripts |
| Argo CD | Pulls approved GitOps changes and deploys them to Kubernetes in a controlled way |

---

## 1.3 Step-by-Step Workflow 3 - TLS Certificate Automation
...
flowchart LR
    ...
    Git[Certificate / Ingress YAML<br/>committed to Git]:::cicd --> Argo[Argo CD applies YAML]:::kube
    Argo --> Cert[Certificate CRD]:::kube
    Cert --> CM[Jetstack cert-manager x3]:::sec
    CM --> Venafi[Venafi VM<br/>Policy + Approval]:::sec
    Venafi --> CA[Enterprise CA / Microsoft CA]:::sec
    CA --> Venafi
    Venafi --> CM
    CM --> Secret[Kubernetes TLS Secret<br/>tls.crt + tls.key]:::sec
    Secret --> Nginx[NGINX Ingress uses cert]:::edge
```

**Why this is used:** certificates are not manually copied to servers. Developers or platform engineers define the certificate requirement in YAML, cert-manager requests it from Venafi, Venafi enforces enterprise PKI policy, and the issued certificate is stored as a Kubernetes TLS Secret for NGINX Ingress.

---

## 1.4 Step-by-Step Workflow 4 - Authentication and Authorization
...
flowchart LR
    ...
    User[Agent / Broker / Staff]:::user --> Nginx[NGINX Ingress]:::kube
    Nginx --> Auth[auth-service x3]:::app
    Auth --> AD[Active Directory DC x2<br/>LDAPS 636]:::sec
    AD --> Auth
    Auth --> Token[JWT / Session Token<br/>Roles and Groups]:::sec
    Token --> App[Policy / Claim / Billing Services]:::app

    Admin[DevOps Admin]:::user --> Rancher[Rancher x3]:::kube
    Rancher --> AD
    Rancher --> RBAC[Kubernetes RBAC<br/>Group-based access]:::sec
```

**Why this is used:** AD is the central identity provider for staff, agents, developers, and administrators. Applications use `auth-service` for login and token creation. Rancher uses AD groups to map users to Kubernetes roles.

---

## 1.5 Step-by-Step Workflow 5 - Observability and Operations
...
flowchart LR
    ...
    Pods[Application Pods]:::app --> Metrics[Metrics endpoint<br/>/actuator/prometheus]:::obs
    Metrics --> Prom[Prometheus StatefulSet x2]:::obs
    Prom --> Graf[Grafana Dashboards x2]:::obs
    Prom --> Alert[Alertmanager x3]:::obs
    Alert --> Team[Email / Teams / Slack]:::user

    Pods --> Logs[stdout / stderr logs]:::obs
    Logs --> Fluent[Fluentd DaemonSet]:::obs
    Fluent --> ES[Elasticsearch StatefulSet x6]:::obs
    ES --> Kib[Kibana x2]:::obs
```

**Why this is used:** Prometheus and Grafana provide metrics, dashboards, and alerts. Fluentd, Elasticsearch, and Kibana provide centralized log collection, search, audit review, and troubleshooting.
