# Day 4 Extension v2: HA VPC + Bastion + Jenkins/SonarQube + ECR + Private EKS + S3/DynamoDB Terraform State
...
## 6A. Add two DynamoDB state-lock tables

> **Current Terraform note:** native S3 lockfiles are now preferred and DynamoDB locking is deprecated by HashiCorp. This lab configures both because you explicitly requested DynamoDB and it is still useful for understanding many existing enterprise Terraform environments. Do not remove `use_lockfile = true`; it is the forward-looking locking mechanism.

Add the following at the end of `bootstrap-state/main.tf`:
...
# PART J — Added EKS, ECR, locking and Kubernetes verification

## 43. Direct answer: does ECR need another state file?
...
## 44. Understanding the three EKS control-plane nodes
...
## 45. Worker scaling from two to six nodes
...
### 46.2 Create a test namespace and deployment
...
### 46.3 Expose the application internally
...
### 46.8 Clean up only the test workload
...
## 47. Complete end-state inventory
...
## 49. Official references used for this update
