# AWS VPC with Public and Private Subnets, NAT Gateway, Jump Host and Private Ubuntu VM
...
> - 1 Internet Gateway
> - 1 public NAT Gateway with an Elastic IP
> - 1 public Ubuntu jump host
> - 1 private Ubuntu web server
> - SSH access from your laptop through the jump host
> - Outbound internet access from the private VM through the NAT Gateway
...
| Public subnet 1 | `us-east-1a` | `10.0.1.0/24` | Jump host and NAT Gateway |
| Public subnet 2 | `us-east-1b` | `10.0.2.0/24` | Reserved for other public resources |
| Private subnet 1 | `us-east-1a` | `10.0.3.0/24` | Private Ubuntu web server |
| Private subnet 2 | `us-east-1b` | `10.0.4.0/24` | Reserved for other private resources |
| Jump host | Public subnet 1 | Dynamic private IP + public IP | Entry point from your laptop |
| Private web VM | Private subnet 1 | Dynamic `10.0.3.x` | Not directly reachable from the internet |
| NAT Gateway | Public subnet 1 | Private IP + Elastic public IP | Outbound internet for private subnets |
...
```mermaid
flowchart TB
    Laptop["💻 Administrator Laptop<br/>Public IP: YOUR_PUBLIC_IP/32"]
    Internet["🌐 Internet"]

    subgraph AWS["AWS Region: us-east-1"]
        direction TB

        subgraph VPC["VPC: openhelp-prod-vpc<br/>CIDR: 10.0.0.0/16"]
            direction TB
            ...
            subgraph AZA["Availability Zone: us-east-1a"]
                ...
                subgraph PUB1["Public Subnet 1<br/>10.0.1.0/24"]
                    Jump["Ubuntu Jump Host<br/>Public IP: assigned by AWS<br/>Private IP: 10.0.1.x<br/>SSH user: ubuntu"]
                    NAT["NAT Gateway<br/>Private IP: 10.0.1.x<br/>Elastic Public IP: assigned by AWS"]
                end

                subgraph PRI1["Private Subnet 1<br/>10.0.3.0/24"]
                    Web["Ubuntu Private Web VM<br/>Private IP: 10.0.3.x<br/>Public IP: none<br/>Nginx installed"]
                end
            end
            ...
            subgraph AZB["Availability Zone: us-east-1b"]
                ...
                subgraph PUB2["Public Subnet 2<br/>10.0.2.0/24"]
                    PublicFuture["Reserved for future<br/>public resources"]
                end

                subgraph PRI2["Private Subnet 2<br/>10.0.4.0/24"]
                    PrivateFuture["Reserved for future<br/>private resources"]
                end
            end

            PublicRT["Public Route Table<br/>10.0.0.0/16 → local<br/>0.0.0.0/0 → Internet Gateway"]
            PrivateRT["Private Route Table<br/>10.0.0.0/16 → local<br/>0.0.0.0/0 → NAT Gateway"]
    ...
    Laptop -->|"SSH TCP/22<br/>allowed only from YOUR_PUBLIC_IP/32"| Jump
    Jump -->|"SSH TCP/22 using private IP"| Web

    Jump -->|"0.0.0.0/0"| IGW
    Web -->|"Outbound request"| NAT
    NAT --> IGW
    IGW <--> Internet

    classDef aws fill:#fff3e0,stroke:#ff9900,stroke-width:3px,color:#111;
    ...
    classDef gateway fill:#fff8e1,stroke:#f57f17,stroke-width:3px,color:#111;
    ...
    class AWS,VPC aws;
    class PUB1,PUB2,Jump,NAT,PublicFuture public;
    class PRI1,PRI2,Web,PrivateFuture private;
...
### 3.3 Private VM to the internet
...
# ---------------------------------------------------------
# Elastic IP for the public NAT Gateway
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# Public NAT Gateway in public subnet 1
# ---------------------------------------------------------
...
Allocates a stable public IPv4 address for the NAT Gateway.
...
Attaches the Elastic IP allocation to the NAT Gateway.

```hcl
subnet_id = aws_subnet.public[0].id
```

Places the NAT Gateway in public subnet 1.

A public NAT Gateway must be in a public subnet because it needs a route through the Internet Gateway.
...
Private-subnet default traffic goes to the NAT Gateway, not directly to the Internet Gateway.

Use `nat_gateway_id`, not `gateway_id`, for a NAT Gateway route.
...
Makes the intended startup order explicit so the private VM is launched after the NAT and private routes are configured.
...
output "nat_gateway_public_ip" {
  description = "Elastic public IPv4 address used by the NAT Gateway"
...
flowchart TD
    ...
    PrivateSubnets --> PrivateAssoc["Private Associations"]
    PrivateRT --> PrivateAssoc

    VPC --> JumpSG["Jump Security Group"]
    VPC --> PrivateSG["Private VM Security Group"]
    JumpSG --> PrivateSG

    PublicAssoc --> Jump["Jump Host"]
    JumpSG --> Jump

    PrivateAssoc --> Web["Private Web VM"]
    PrivateSG --> Web
    NAT --> Web
    ...
    classDef gateway fill:#fff8e1,stroke:#f57f17,stroke-width:2px,color:#111;

    class VPC,PublicSubnets,PrivateSubnets,PublicRT,PrivateRT,PublicAssoc,PrivateAssoc network;
    class JumpSG,PrivateSG security;
    class Jump,Web compute;
    class IGW,EIP,NAT gateway;
```
...
nat_gateway_public_ip   = "3.90.50.40"
```
...
The two values should match. This demonstrates that the private VM's outbound connection is translated to the NAT Gateway Elastic IP.
...
## 37. SSH through the jump host fails
...
## 38. Private VM cannot access the internet

Check:

1. The NAT Gateway state:
...
2. The NAT Gateway is in a public subnet.
3. The NAT Gateway has an Elastic IP.
4. The public subnet route table uses the Internet Gateway.
5. The private route table uses the NAT Gateway.
6. The private subnet is associated with the private route table.
7. The private security group permits outbound traffic.
8. VPC DNS support is enabled.

---

## 39. Nginx was not installed
...
## 42. One NAT Gateway versus two

This beginner lab uses one NAT Gateway:
...
- The second Availability Zone depends on a NAT Gateway in the first zone.
- Cross-AZ traffic may add cost.
- Loss of the NAT Gateway's AZ can interrupt outbound internet for both private subnets.

A production high-availability design normally uses one NAT Gateway per Availability Zone:

```text
Private subnet 1 → NAT Gateway 1 in us-east-1a
Private subnet 2 → NAT Gateway 2 in us-east-1b
```
...
## 43. Better alternatives to a permanent jump host
...
| `aws_route_table.private` | 1 | Routes private outbound traffic to NAT |
| `aws_eip.nat` | 1 | Stable public IPv4 for NAT |
| `aws_nat_gateway` | 1 | Private-subnet outbound internet |
...
6. [AWS NAT Gateway use cases](https://docs.aws.amazon.com/vpc/latest/userguide/nat-gateway-scenarios.html)
7. [Connect to a Linux EC2 instance using SSH](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/connect-linux-inst-ssh.html)
8. [Canonical: Find Ubuntu images on AWS](https://documentation.ubuntu.com/aws/aws-how-to/instances/find-ubuntu-images/)
9. [Terraform AWS provider: VPC](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/vpc)
10. [Terraform AWS provider: Internet Gateway](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/internet_gateway)
11. [Terraform AWS provider: NAT Gateway](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/nat_gateway)
12. [Terraform AWS provider: Elastic IP](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/eip)
13. [Terraform AWS provider: Route table](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/route_table)
14. [Terraform AWS provider: EC2 instance](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/instance)
15. [Terraform AWS provider: Security group](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/security_group)
...
- [ ] NAT outbound IP matches NAT Elastic IP
- [ ] Nginx page works
