# Day 5 Final Guide: HA VPC + Bastion + Jenkins/SonarQube + ECR + Private EKS + S3/DynamoDB Terraform State

> **Beginner-friendly, production-style Terraform extension**
>
> This final guide builds a complete beginner-friendly AWS environment while preserving the original two-AZ VPC design.
> It keeps:
>
> - 1 VPC: `10.0.0.0/16`
...
> - AWS-managed highly available EKS control plane across three Availability Zones
> - At least two managed Kubernetes API server instances and three managed `etcd` instances; these are not customer-visible EC2 instances
> - EKS managed worker node group with 4 desired nodes, minimum 2 and maximum 6, distributed across both private subnets
...
>   - One for EKS/ECR Terraform state
> - Native S3 state locking using Terraform `use_lockfile = true`
> - Two DynamoDB lock tables, one per Terraform state, for compatibility and traditional locking practice
> - One private Amazon ECR repository for Jenkins-built application images
> - ECR scan-on-push, immutable image tags, lifecycle cleanup and KMS encryption
...
1. Create the folders and files exactly as shown.
2. Copy each complete code block into the matching filename.
3. Change only values clearly marked as account-specific, such as the AWS account ID, AMI ID, key-pair name and administrator `/32` CIDR.
4. Run the modules in this order: `bootstrap-state` → `ec2-infra` → `eks-infra`.

No section below is an incremental patch. Every filename section represents the final contents or the complete required portion of that file.
...
> The first image is the final source-of-truth architecture. The older image is retained only for comparison and historical reference.

<img width="1535" height="1024" alt="Original Day 4 architecture" src="https://github.com/user-attachments/assets/bf46d650-0039-4a64-809f-371935f9c929" />
...
| EKS worker EC2 instances | 4 desired; 2 minimum; 6 maximum | Private subnets |
| ECR private repository | 1 | Regional |
| DynamoDB lock tables | 2 | One per Terraform state |
...
+ 2 to 6 private EKS worker EC2 instances
= 4 to 8 EC2 virtual machines

Normal desired capacity: 2 + 4 = 6 EC2 virtual machines
```

The EKS control plane is managed by AWS and does not appear as EC2 instances in your account. You do not create or manage three control-plane EC2 hosts yourself. EKS operates a highly available control plane across multiple Availability Zones. The closest EKS equivalent to your three-control-plane-node requirement is AWS-managed high availability: EKS runs the control plane across three Availability Zones with at least two API server instances and three `etcd` instances. You do not choose or manage exactly three control-plane EC2 hosts.
...
The repository is organised into three complete Terraform root modules. Each file shown below is the final copy-ready version for its directory.
...
This file pins the Terraform CLI and AWS provider versions. The provider block also applies consistent project, environment and layer tags to every supported AWS resource.

`use_lockfile = true` requires Terraform 1.10 or newer, which is why the minimum Terraform version is declared here.
...
variable "ec2_lock_table_name" {
  description = "DynamoDB table used for EC2/VPC state locking"
  type        = string
  default     = "openhelp-prod-ec2-tf-lock"
}

variable "eks_lock_table_name" {
  description = "DynamoDB table used for EKS/ECR state locking"
  type        = string
  default     = "openhelp-prod-eks-tf-lock"
}
```

### Brief explanation

These variables collect reusable names, the AWS Region and the two lock-table names. The S3 bucket values have no default because every AWS account must supply globally unique names.

---

## 6. `bootstrap-state/main.tf` — S3 and KMS resources
...
# ---------------------------------------------------------
# Traditional lock table for EC2/VPC state
# Terraform expects a String partition key named LockID.
# ---------------------------------------------------------
resource "aws_dynamodb_table" "ec2_state_lock" {
  name         = var.ec2_lock_table_name
  ...
  tags = {
    Name    = var.ec2_lock_table_name
    Purpose = "Terraform lock for EC2 and VPC state"
  }
}

# ---------------------------------------------------------
# Traditional lock table for EKS/ECR state
# ---------------------------------------------------------
resource "aws_dynamodb_table" "eks_state_lock" {
  name         = var.eks_lock_table_name
  billing_mode = "PAY_PER_REQUEST"
  hash_key     = "LockID"
  ...
  tags = {
    Name    = var.eks_lock_table_name
    Purpose = "Terraform lock for EKS and ECR state"
  }
}
```

### Beginner map for the S3 security blocks

```yaml
public_access_block:
  purpose: Keeps the state bucket private
  result: Public ACLs and public policies cannot expose it

ownership_controls:
  purpose: Makes the bucket account own every state object
  result: ACL-based object ownership is disabled

bucket_policy:
  purpose: Requires encrypted HTTPS transport
  result: Every non-HTTPS S3 request is denied
```

```mermaid
flowchart LR
    TF["Terraform request"] --> PUB["Public access block<br/>Bucket stays private"]
    PUB --> OWN["Ownership controls<br/>Bucket owner controls objects"]
    OWN --> TLS{"HTTPS used?"}
    TLS -- Yes --> IAM["Continue IAM and KMS checks"]
    TLS -- No --> DENY["Deny request"]
```

The public-access block controls whether the bucket can become public. Ownership controls decide who owns uploaded objects, while the bucket policy independently rejects insecure HTTP connections.
...
### Native S3 locking and DynamoDB compatibility

The final `main.tf` above creates both DynamoDB compatibility tables, while each backend uses `use_lockfile = true` for native S3 locking. HashiCorp now prefers S3 lockfiles and deprecates DynamoDB locking, so DynamoDB is retained here only for migration compatibility and for understanding older enterprise designs.
...
output "ec2_lock_table_name" {
  description = "DynamoDB compatibility lock table for EC2/VPC state"
  value       = aws_dynamodb_table.ec2_state_lock.name
}

output "eks_lock_table_name" {
  description = "DynamoDB compatibility lock table for EKS/ECR state"
  value       = aws_dynamodb_table.eks_state_lock.name
}
```

### Brief explanation

These outputs expose the bucket names, KMS key ARNs and compatibility lock-table names needed by later Terraform modules and validation commands.
...
# The AWS account ID provides a simple globally unique suffix.
ec2_state_bucket_name = "openhelp-prod-123456789012-ec2-tfstate"
eks_state_bucket_name = "openhelp-prod-123456789012-eks-tfstate"

ec2_lock_table_name = "openhelp-prod-ec2-tf-lock"
eks_lock_table_name = "openhelp-prod-eks-tf-lock"
```

Determine your AWS account ID with:
...
This file supplies the final bootstrap names. The account ID in each S3 bucket name makes the globally unique name easy to identify, while the lock-table names are regional and do not need global uniqueness.
...
Backend blocks cannot use normal Terraform variables. This final example uses explicit backend values; the same values can alternatively be supplied with `terraform init -backend-config`.
...
terraform {
  backend "s3" {
    ...
    use_lockfile  = true
    dynamodb_table = "openhelp-prod-ec2-tf-lock"
  }
}
```
...
This backend stores the EC2/VPC state remotely, encrypts it with the EC2 state KMS key and enables native S3 locking. The DynamoDB table remains configured only for compatibility with older Terraform workflows.
...
This file creates the VPC, Internet Gateway, two public subnets, two private subnets, two NAT Gateways and their route tables. Each private subnet uses the NAT Gateway in the same Availability Zone, and the EKS subnet tags help AWS load balancers identify public and private subnets.
...
The bastion security group permits SSH only from approved administrator CIDRs. The tools security group permits SSH, Jenkins and SonarQube only from those same CIDRs, while both instances retain outbound access for package and AWS API communication.
...
# Jenkins can authenticate, push and pull container images.
# For a production environment, replace this broad managed policy with a repository-scoped custom policy.
# =========================================================
...
The bastion instance receives AWS administration tools and its IAM instance profile. The tools instance installs Docker, Jenkins, kubectl and a persistent-volume-backed SonarQube lab container; both instances require IMDSv2 and encrypted gp3 root volumes.
...
# Required: current Canonical Ubuntu 24.04 AMI ID in us-east-1.
ubuntu_ami_id = "ami-REPLACE_ME"

key_name = "openhelp-key"

# Required: your current public IPv4 address followed by /32.
admin_cidr_blocks = [
  "83.24.100.50/32"
]
...
terraform {
  backend "s3" {
    ...
    use_lockfile   = true
    dynamodb_table = "openhelp-prod-eks-tf-lock"
  }
}
```
...
This backend stores EKS and ECR state separately from the VPC/EC2 state. It uses the EKS state KMS key, native S3 locking and the EKS compatibility DynamoDB table.
...
These variables define the EKS version, remote EC2-state location, API access CIDRs and worker-node scaling limits. Validation prevents an unrestricted `0.0.0.0/0` EKS public endpoint.

> The example uses Kubernetes `1.36`, which is supported by Amazon EKS as of July 2026. Verify supported versions and add-on compatibility immediately before deployment.
...
This file reads only the published outputs from the EC2/VPC state, then exposes them as local values for the EKS module. It also discovers the current AWS account and partition so ARNs work in standard and non-standard AWS partitions.
...
This file creates the EKS secrets KMS key, control-plane log group, managed cluster, managed worker group, access entries and core EKS add-ons. Worker nodes remain private, while the public API endpoint is restricted to approved CIDRs and private endpoint access is enabled.
...
ECR does **not** need another Terraform state file in this design. It belongs to the EKS/application-delivery layer and is stored in the existing EKS state bucket. A separate third state is only justified when another team independently owns ECR or when its lifecycle must be completely independent.

```hcl
# ---------------------------------------------------------
# KMS key for ECR images
# ---------------------------------------------------------
resource "aws_kms_key" "ecr" {
  description             = "KMS key for ${var.project_name} ECR images"
  deletion_window_in_days = 7
  enable_key_rotation     = true
...
# ---------------------------------------------------------
# Private application image repository
# ---------------------------------------------------------
resource "aws_ecr_repository" "application" {
  ...
  tags = {
    Name    = "${var.project_name}-${var.environment}-application"
    Purpose = "Jenkins-built application images for EKS"
  }
}

# Keep releases, but automatically clean untagged build leftovers.
...
This file creates a private, KMS-encrypted ECR repository with immutable image tags and scan-on-push. The lifecycle policy removes only untagged images older than 14 days, leaving tagged releases intact.
...
sequenceDiagram
    ...
    participant EKS as EKS worker nodes

    Dev->>Jenkins: Git push triggers pipeline
    Jenkins->>Sonar: Run code-quality analysis
    Sonar-->>Jenkins: Quality gate result
    Jenkins->>Jenkins: Build Docker image with unique tag
    Jenkins->>ECR: Authenticate and push image
    Jenkins->>EKS: Update Kubernetes Deployment image
    EKS->>ECR: Pull immutable image
    EKS-->>Jenkins: Rolling deployment status
```

### Example Jenkins shell commands for ECR

```bash
AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text)
AWS_REGION=us-east-1
ECR_REPOSITORY=openhelp/prod/application
...
kubectl set image deployment/openhelp-web \
  openhelp-web=${ECR_URI}:${IMAGE_TAG} \
  -n openhelp

kubectl rollout status deployment/openhelp-web -n openhelp
```

## 25. `eks-infra/outputs.tf`
...
output "ecr_repository_url" {
  description = "ECR repository URL used by Jenkins and Kubernetes"
...
## 29. Deploy VPC and the two public EC2 instances

After setting the account-specific backend, AMI, key-pair and administrator CIDR values, run:
...
## 31. Check Jenkins and SonarQube
...
## 32. Deploy EKS second

After setting the account-specific EKS backend and administrator CIDR values, run:
...
## 33. Connect to EKS from the bastion
...
## 34. Connect to EKS from Jenkins
...
## 35. Validate route-table design
...
## 38. Security improvements already included
...
## 39. Recommended future production changes
...
11. Add separate ECR repositories and enhanced scanning policies for additional applications.
...
## 43. Final design explanation for interviews
...
# PART J — EKS, ECR, locking and Kubernetes verification

## 44. Direct answer: does ECR need another state file?

No. In this exercise, use exactly these two remote states:

| State | Resources owned | Locking |
|---|---|---|
| EC2/VPC state | VPC, subnets, routes, NAT Gateways, bastion, Jenkins/SonarQube and their IAM profiles | Native S3 lockfile plus `openhelp-prod-ec2-tf-lock` |
| EKS state | EKS control plane configuration, node group, add-ons, EKS access entries, ECR repository and related KMS keys | Native S3 lockfile plus `openhelp-prod-eks-tf-lock` |

Create a third state only when ECR has a different owner, deployment schedule, access boundary or destruction lifecycle. More state files increase dependency handling, IAM permissions and operational work, so separation should be intentional.

---

## 45. Understanding the three EKS control-plane nodes

With self-managed Kubernetes, you would normally create three control-plane virtual machines for quorum and high availability. With Amazon EKS, AWS creates and operates the control-plane infrastructure across three Availability Zones. EKS provides one highly available Kubernetes API endpoint backed by at least two API server instances and three `etcd` instances. These run in an AWS-managed account and are not displayed as EC2 instances in your account.
...
- Worker node groups
- Add-ons and workloads

What AWS manages:
- Kubernetes API server infrastructure
- etcd control-plane data store infrastructure
- Control-plane high availability and replacement
- Control-plane patching as part of the EKS service
```

Therefore, do not create three EC2 instances and attempt to join them as EKS control-plane nodes. The visible EC2 instances are only the 2–6 worker nodes plus the bastion and tools hosts.

---

## 46. Worker scaling from two to six nodes

The node group starts with four workers and is bounded between two and six:
...
These values establish limits but do not by themselves react to unschedulable pods. For automatic scale-out and scale-in based on pod demand, install Cluster Autoscaler or Karpenter. For this beginner exercise, first validate manual scaling:
...
aws eks update-nodegroup-config \
  ...
  --cluster-name openhelp-prod-eks \
  --nodegroup-name openhelp-prod-eks-general \
  --scaling-config minSize=2,maxSize=6,desiredSize=5
```
...
> When a Kubernetes autoscaler controls desired capacity, avoid repeatedly forcing `desired_size` from Terraform. A production configuration commonly uses `lifecycle.ignore_changes` for the autoscaler-managed desired value.

---

## 47. Basic Kubernetes test exercise

Run these commands from the bastion after `aws eks update-kubeconfig`.

### 47.1 Confirm cluster health
...
### 47.2 Create a test namespace and deployment

```bash
kubectl create namespace openhelp

kubectl create deployment openhelp-web \
  --image=nginx:1.27-alpine \
  --replicas=3 \
  -n openhelp

kubectl get deployment,pods -n openhelp -o wide
```

### 47.3 Expose the application internally

```bash
kubectl expose deployment openhelp-web \
  --name openhelp-web \
  --port 80 \
  --target-port 80 \
  --type ClusterIP \
...
SERVICE_IP=$(kubectl get service openhelp-web -n openhelp -o jsonpath='{.spec.clusterIP}')
...
### 47.4 Test self-healing

```bash
kubectl get pods -n openhelp
POD_NAME=$(kubectl get pods -n openhelp -o jsonpath='{.items[0].metadata.name}')
kubectl delete pod ${POD_NAME} -n openhelp
kubectl get pods -n openhelp -w
```

The Deployment creates a replacement pod automatically. Press `Ctrl+C` after the new pod becomes `Running`.

### 47.5 Test scaling and rollout

```bash
kubectl scale deployment openhelp-web --replicas=5 -n openhelp
kubectl get pods -n openhelp -o wide
...
kubectl rollout status deployment/openhelp-web -n openhelp
kubectl rollout history deployment/openhelp-web -n openhelp
```

### 47.6 Roll back a deployment

```bash
kubectl rollout undo deployment/openhelp-web -n openhelp
kubectl rollout status deployment/openhelp-web -n openhelp
```

### 47.7 Inspect and troubleshoot

```bash
kubectl describe deployment openhelp-web -n openhelp
kubectl describe pod POD_NAME -n openhelp
kubectl logs POD_NAME -n openhelp
kubectl get endpoints openhelp-web -n openhelp
kubectl top nodes
kubectl top pods -n openhelp
```

`kubectl top` requires Metrics Server. Install it if those commands report that the Metrics API is unavailable.

### 47.8 Clean up only the test workload

```bash
kubectl delete namespace openhelp
```

---

## 48. Complete end-state inventory

After the exercise, the intended running environment is:

```text
Standalone EC2:
1. openhelp-prod-bastion
2. openhelp-prod-jenkins-sonarqube

EKS:
3. openhelp-prod-eks managed control plane
4. openhelp-prod-eks-general managed node group
5. Between 2 and 6 private worker EC2 instances; desired count 4

Container registry:
6. openhelp/prod/application ECR repository

Terraform backend:
7. EC2/VPC S3 state bucket
8. EKS/ECR S3 state bucket
9. EC2/VPC DynamoDB lock table
10. EKS/ECR DynamoDB lock table
```

Verify everything:

```bash
terraform -chdir=bootstrap-state output
terraform -chdir=ec2-infra output
terraform -chdir=eks-infra output
...
aws ecr describe-repositories \
  --region us-east-1 \
  --repository-names openhelp/prod/application \
  --output table
...
## 49. Final deployment flow

```mermaid
flowchart TD
    A[1. bootstrap-state terraform apply] --> B[Creates 2 S3 buckets + 2 DynamoDB lock tables + KMS keys]
    B --> C[2. ec2-infra terraform init/apply]
    C --> D[Creates VPC, routes, NATs, bastion, Jenkins/SonarQube]
    D --> E[3. eks-infra terraform init/apply]
    E --> F[Creates EKS, 2-6 worker node group, add-ons and ECR]
    F --> G[4. Configure kubeconfig on bastion and Jenkins]
    G --> H[5. Build image, push to ECR and deploy to EKS]
    H --> I[6. Run Kubernetes validation commands]
```
...
- Both S3 backend bucket names include your real AWS account ID.
- Both backend files use the same bucket, key, Region, KMS alias and lock-table names created by `bootstrap-state`.
- `ubuntu_ami_id` is a real Ubuntu 24.04 AMI in `us-east-1`.
- `key_name` already exists in `us-east-1`.
- Every administrator CIDR is your current public IP with `/32`, never `0.0.0.0/0`.
- `bootstrap-state` is applied before either remote backend is initialised.
- `ec2-infra` is applied before `eks-infra` because EKS reads the VPC outputs.
- The state buckets are backed up before any intentional bootstrap destruction.
...
- Amazon EKS Kubernetes 1.36 release notes: <https://docs.aws.amazon.com/eks/latest/userguide/kubernetes-versions-standard.html>
...
> Verify supported EKS versions and compatible add-on versions immediately before deployment. The guide uses EKS Kubernetes `1.36`, which Amazon EKS introduced in June 2026.
