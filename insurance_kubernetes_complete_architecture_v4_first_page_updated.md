...
The first architecture diagram is updated to use **NGINX Ingress direct routing** instead of a separate API Gateway layer. In this model, **F5 HA Pair / external load balancer sends HTTPS traffic to the NGINX Ingress Controller**, and NGINX Ingress routes directly to Kubernetes **ClusterIP Services** such as `customer-service`, `policy-service`, `claims-service`, and `billing-service`.
...
flowchart LR
    ...
    LBSVC --> NGINX[NGINX Ingress Controller Pods x3<br/>Path-based routing + TLS]:::kube
...
### First page architecture traffic path
...
### Why API Gateway is removed from the first diagram

In this updated design, **NGINX Ingress Controller is acting as the routing gateway**. It performs HTTPS entry, TLS termination, path-based routing, and load balancing to the Kubernetes Services. A separate API Gateway product such as Kong, Apigee, or Spring Cloud Gateway can still be added later if advanced API management is required, but it is not mandatory for this Kubernetes routing design.

### CI/CD and tool flow included in first architecture
...
### Nexus usage in this architecture

| Nexus Repository Type | What it stores | Used by |
|---|---|---|
| Docker registry | Microservice container images | Kubernetes Deployments |
| Maven repository | Java JAR dependencies and build artifacts | Jenkins builds |
| Helm repository | Helm charts for application deployment | Argo CD / Helm |
| Raw repository | Scripts, config bundles, release packages | DevOps team |
